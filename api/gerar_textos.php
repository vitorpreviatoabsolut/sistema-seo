...
try {
    ...
    foreach ($keywords as $keyword) {
        foreach ($regions as $region) {
            ...
            $contentRaw = $jsonResponse['candidates'][0]['content']['parts'][0]['text'] ?? null;

            if ($contentRaw) {
                $contentObj = json_decode($contentRaw, true);
                $seoText = $contentObj['seoText'] ?? $contentRaw;
                $metaDesc = $contentObj['metaDescription'] ?? '';

                // Prepara o conteúdo do arquivo final (usando o template salvo no banco ou padrão)
                $templateContent = $client['template_content'] ?? '<!DOCTYPE html><html><body><h1>{{TITLE}}</h1>{{SEO_TEXT}}</body></html>';
