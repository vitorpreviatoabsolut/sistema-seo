...
$resource = $parts[0] ?? ''; // ex: clients
$id = $parts[1] ?? null;     // ex: 1
$subResource = $parts[2] ?? null; // ex: keywords
$action = $parts[3] ?? null; // ex: bulk
...
try {
    if ($resource === 'clients') {
        ...
        elseif ($method === 'POST' && !$id) {
            ...
            $stmt->execute([$data['name'], $data['context'] ?? '']);
        ...
        elseif ($method === 'PUT' && $id && $subResource === 'whatsapp') {
            ...
            $stmt->execute([$data['whatsapp_number'], $data['whatsapp_message'], $id]);
            echo json_encode(['success' => true]);
        }
    }
